                ...
                <div class="mb-9 sm:flex sm:gap-32 lg:mb-12">
                    <div>
                        <ul class="grid gap-3 mb-11 lg:gap-2 lg:mb-0">
                            <li>Native Name: <span class="text-sm font-light lg:text-base">${country.name.nativeName[Object.keys(country.name.nativeName)[0]].common}</span></li>
                            <li>Population: <span class="text-sm font-light lg:text-base">${country.population.toLocaleString()}</span></li>
                            <li>Region: <span class="text-sm font-light lg:text-base">${country.region}</span></li>
                            <li>Sub Region: <span class="text-sm font-light lg:text-base">${country.subregion}</span></li>
                            <li>Capital: <span class="text-sm font-light lg:text-base">${country.capital}</span></li>
                        </ul>    
                    </div>
                    
                    <div>
                        <ul class="grid gap-3 lg:gap-2">
                            <li>Time Zone<small>(s)</small>:<span class="text-sm font-light lg:text-base">${country.timezones.map(timeZone => " " + timeZone).slice(0, 2)}</span></li>
                            <li>Top Level Domain<small>(s)</small>:<span class="text-sm font-light lg:text-base">${country.tld.map(tld => " " + tld).slice(0, 2)}</span></li>
                            <li>Currencies: <span class="text-sm font-light lg:text-base">${country.currencies[Object.keys(country.currencies)[0]].name}</span></li>
                            <li>Official Language<small>(s)</small>:<span class="text-sm font-light lg:text-base">${Object.values(country.languages).map(languageName => ' ' + languageName)}</span></li>
                        </ul>    
                    </div>
                </div>
